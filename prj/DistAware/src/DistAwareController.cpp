...
DistAwareController::DistAwareController(std::shared_ptr<RobotWorldModel> _wm) : Controller(_wm)
{
...
DistAwareController::~DistAwareController()
{
    // nothing to do.
}

void DistAwareController::reset()
{
    // nothing to do.
}
        ...
        for ( int i = 0 ; i != NB_SENSORS ; i++ ) // also possible to check each sensor separatly using SENSOR_L, SENSOR_R, etc. (check Utilities/Sensorbelt.h)
        {
            ...
            else
            {
                ...
                if ( robotId != -1 )
                {
                    ...
                    auto targetRobotController = std::dynamic_pointer_cast<DistAwareController>(getRobotControllerAt(i));
...
{
    auto other = gWorld->getRobot(id)->getController();
